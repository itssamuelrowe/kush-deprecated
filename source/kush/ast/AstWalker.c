    ...
    if (node != NULL) {
        if (k_ASTNode_isErroneous(node)) {
            ...
            switch (listener->m_walkerState) {
                case KUSH_AST_WALKER_STATE_VISIT_CHILDREN: {
                ...
                case KUSH_AST_WALKER_STATE_SKIP_CHILDREN: {
                    // Skip the children! Nothing to do here.

                    break;
                }

                case KUSH_AST_WALKER_STATE_VISIT_FIRST_CHILD: {
                    if (size > 0) {
                ...
                case KUSH_AST_WALKER_STATE_VISIT_LAST_CHILD: {
                    if (size > 0) {
            ...
            // please process it here. Further, do not reset to KUSH_AST_WALKER_STATE_VISIT_CHILDREN.
...
void k_ASTWalker_enterRule(k_ASTListener_t* listener, k_ASTNode_t* node) {
    ...
    switch (node->m_type) {
        ...
        case KUSH_AST_NODE_TYPE_PRIMARY_EXPRESSION: {
            listener->m_onEnterPrimaryExpression(listener, node);
            break;
        }

        case KUSH_AST_NODE_TYPE_MAP_EXPRESSION: {
            listener->m_onEnterMapExpression(listener, node);
            break;
        }

        case KUSH_AST_NODE_TYPE_MAP_ENTRIES: {
            listener->m_onEnterMapEntries(listener, node);
            break;
        }

        case KUSH_AST_NODE_TYPE_MAP_ENTRY: {
            listener->m_onEnterMapEntry(listener, node);
            break;
        }

        case KUSH_AST_NODE_TYPE_LIST_EXPRESSION: {
            listener->m_onEnterListExpression(listener, node);
            break;
        }

        case KUSH_AST_NODE_TYPE_NEW_EXPRESSION: {
